...
class FewShotNERModel(nn.Module):
    ...
    def __transform_label_to_tag__(self, pred, query):
        ...
        for idx, num in enumerate(query['sentence_num']):
            ...
            pred_tag += [query['label2tag'][idx][label] for label in
                         pred[current_token_idx:current_token_idx + set_token_length]]
...
class Proto(FewShotNERModel):
    def __init__(self, word_encoder, dot=False, ignore_index=-1):
        FewShotNERModel.__init__(self, word_encoder, ignore_index=ignore_index)
...
class FewShotNERFramework:
    def __init__(self, train_data_loader, val_data_loader, test_data_loader, viterbi=False, N=None, train_fname=None,
                 tau=0.05, use_sampled_data=True):
        '''
